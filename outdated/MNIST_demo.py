...
class SCACNN(nn.Module):
    def __init__(self, contextNN, c_len):
        ...
        self.scaconv1 = SCAConv(1, 2, kernel_size=3, padding=1, stride=2, b=self.b, c_len=self.c_len, mlp_hidden=h, condition=True)
        self.scaconv2 = SCAConv(2, 2, kernel_size=3, padding=1, stride=2, b=self.b, c_len=self.c_len, mlp_hidden=h, condition=True)
        self.relu = nn.ReLU()
...
with torch.no_grad():
    ...
    for images, labels in test_loader:
        ...
        k1 = model.scaconv1.kernel.detach().clone()
        k2 = model.scaconv2.kernel.detach().clone()
        model.scaconv1.kernel -= k1
        model.scaconv2.kernel -= k2
...
print(f'Adaptive-only SCA-CNN accuracy on the 10,000 test images: {adaptive_accuracy:.2f}%')
...
with open(r"Spatial-Contextual-Adaptive-Convolution\Development\Outputs\SCA_CNN_results.txt", "w") as f:
    ...
    f.write(f"Adaptive-only SCA-CNN - Accuracy on the 10,000 test images: {adaptive_accuracy:.2f}%\n\n")
